...
def send_message():
  ...
  for i in chat_list:
    if i['session_id'] == rid:
      ...
      try:
        # Last message
        cursor = g.conn.execute("SELECT * FROM chat WHERE session_id =  (%s)", rid)
        last_message = ''
        counter = 1
        for n in cursor:
          counter += 1
          print(counter)
        cursor.close()
        cursor = g.conn.execute("INSERT INTO chat VALUES (%s, %s, %s, %s, %s, %s)", rid, counter, date, message, id, recipient)
        cursor.close()
      except:
        last_message = "No messages..."
  chat_list.close()
